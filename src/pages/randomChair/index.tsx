...
import { shuffle, transformText } from '../../utils';
    ...
    <div className="App">
      <header className="App-header">
        ...
        <form className="button">
          <div>
            {edit && (
              <>
                ...
                <button onClick={handleSave} className="button__submit-changes">
                  <i className="fa fa-check" aria-hidden="true"></i>
                </button>
              </>
            )}
          </div>
          {!edit && numbers.length !== 10 && (
            <input type="text" value={userName} onChange={handleChange} ref={refInput}></input>
          )}

          {numbers.length !== 10 && !edit && (
            <button className="button__random" onClick={getNumber} disabled={numbers.length === 10}>
              random
            </button>
          )}
          ...
          {numbers.length === 10 && !edit && (
            <Button variant="contained" onClick={shake}>
              Перемешать
            </Button>
          )}
        </form>
